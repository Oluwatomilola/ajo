...
contract PiggyBankTest is Test {
    ...
    function testMultiUserFixture() public {
        MultiUserFixture memory fixture = createMultiUserFixture();

        // Verify all users can deposit
        assertEq(fixture.user1.balance, 10 ether);
        assertEq(fixture.user2.balance, 10 ether);
        assertEq(fixture.user3.balance, 10 ether);

        // Users make deposits
        vm.prank(fixture.user1);
        piggyBank.deposit{value: 1 ether}();

        vm.prank(fixture.user2);
        piggyBank.deposit{value: 2 ether}();

        vm.prank(fixture.user3);
        piggyBank.deposit{value: 3 ether}();
    ...
    function createMultiUserFixture()
        internal
        returns (MultiUserFixture memory)
    {
        return
            MultiUserFixture({
                user1: address(0x10),
